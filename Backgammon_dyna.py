...
def play_a_game(commentary=False, net=None, train=False):
        ...
        for i in range(1 + int(dice[0] == dice[1])):
            ...
            if player == 1:
                if(count < 2):
                    ...
                    new_move = net.actor.a
                    # new_state = net.actor.s_prime
                else:
                    new_move, new_state = net.actor.nextMove(board_copy, dice, player, net.search.theta)
            ...
            if len(move) != 0:
                ...
                if(player == 1 and count > 1):
                    ...
                    if(not game_over(new_state) and not check_for_error(new_state)):
                        ...
                        # net.actor.a_prime, net.actor.s_prime_prime = net.actor.nextMove(board, dice, player, net.search.theta)
                        # if(len(net.actor.a_prime) == 0 or len(net.actor.s_prime_prime) == 0):
                        #    check_no_move = 1
                        # else:
                        if(train is True):
                            delta = 0 + agent.sigmoid(net.actor.getValue(agent.getFeatures(new_state, player), net.search.theta)) - agent.sigmoid(net.actor.getValue(agent.getFeatures(old_state, player), net.search.theta))
                        ...
                        # old_move = # net.actor.a
                        old_state = new_state  # net.actor.s
                    # else:
                    #     break
    ...
    # return the winner
    if(train is True):
        delta = player * -1 + 0 - agent.sigmoid(net.actor.getValue(agent.getFeatures(old_state, player * -1), net.search.theta))
        net.actor.theta = net.actor.theta + net.actor.alpha * delta * net.actor.z
